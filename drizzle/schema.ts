...
export const userProfiles = mysqlTable("user_profiles", {
  id: int("id").autoincrement().primaryKey(),
  userId: int("userId").notNull().unique(),
  profileImage: varchar("profileImage", { length: 500 }), // S3 URL or storage key
  bio: text("bio"),
  headline: varchar("headline", { length: 255 }), // e.g., "Full Stack Developer"
  skills: text("skills"), // JSON stringified array
  experience: text("experience"), // JSON stringified array
  education: text("education"), // JSON stringified array
  location: varchar("location", { length: 255 }),
  website: varchar("website", { length: 500 }),
  socialLinks: text("socialLinks"), // JSON stringified object
  resumeFileKey: varchar("resumeFileKey", { length: 500 }), // S3 storage key
...
export const userSubscriptions = mysqlTable("user_subscriptions", {
  ...
  monthlyFee: int("monthlyFee").default(0), // in cents (300 = $3.00 or equivalent currency)
...
export const newsletterSubscribers = mysqlTable("newsletter_subscribers", {
  ...
  userId: int("userId"), // Optional: link to user if they're logged in
 ...
 * Tracks all payments for subscriptions
 */
export const paymentHistory = mysqlTable("payment_history", {
  id: int("id").autoincrement().primaryKey(),
  userId: int("userId").notNull(),
  subscriptionId: int("subscriptionId").notNull(),
  stripePaymentId: varchar("stripePaymentId", { length: 255 }), // Stripe payment ID
  amount: int("amount").notNull(), // in cents
  currency: varchar("currency", { length: 10 }).default("USD"),
  status: mysqlEnum("status", ["pending", "completed", "failed", "refunded"]).default("pending").notNull(),
  paymentMethod: varchar("paymentMethod", { length: 100 }), // e.g., "card", "bank_transfer"
